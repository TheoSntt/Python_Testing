class TestBookingClass:
    ...
    def test_booking_max_allowed_places_respected_in_ui(self, client):
        ...
        # Competition : 50 places  | Club : 20 points | Booked : 0
        response = client.get('/book/competition_test1/club_test1')
        assert response.status_code == 200
        data = response.data.decode()
        assert 'name="places" value="1" min="1" max="20"' in data

        # Competition : 15 places  | Club : 20 points | Booked : 0
        response = client.get('/book/competition_test2/club_test1')
        data = response.data.decode()
        assert 'name="places" value="1" min="1" max="15"' in data

        # Competition : 6 places  | Club : 20 points | Booked : 0
